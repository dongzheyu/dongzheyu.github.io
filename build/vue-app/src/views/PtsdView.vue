<template>
  <TestTemplate
    title="创伤后应激障碍（PTSD）自评测试"
    subtitle="PTSD 检查表（PCL-5） · 20 道题 · 约 6 分钟"
    description="PTSD 检查表（Posttraumatic Stress Disorder Checklist for DSM-5）是根据 DSM-5 标准开发的自评工具，用于评估创伤后应激障碍症状。请根据过去一个月的情况回答。"
    instruction="以下问题涉及你在经历创伤事件后可能出现的反应。请根据过去一个月的情况，评估每种问题对你的困扰程度："
    test-id="pcl5"
    test-title="PTSD自评测试"
    :questions="questions"
    :options="freqOptions"
    :calculate-result-fn="calculateResult"
    theme-class="ptsd-page"
    hero-padding="5%"
    show-answer-review
    :dimensions="dimensionData"
    disclaimer="PCL-5 是筛查工具，不能替代临床诊断。PTSD 是一种可能影响任何人（无论创伤事件类型）的精神障碍，通过适当的治疗（如眼动脱敏与再处理疗法、认知加工疗法）通常能够显著改善。如果你经历过创伤事件并持续感到困扰，请寻求专业帮助。"
  />
</template>
...
const dimensionData = computed(() => [
  {
    name: '再体验症状',
    icon: 'mdi:eye',
    description: '侵入性回忆、噩梦、闪回',
    score: clusterB.value,
    maxScore: 20,
  },
  {
    name: '回避症状',
    icon: 'mdi:cancel',
    description: '回避创伤相关刺激',
    score: clusterC.value,
    maxScore: 8,
  },
  {
    name: '负性认知情绪',
    icon: 'mdi:lightning-bolt',
    description: '认知扭曲、负性情绪',
    score: clusterD.value,
    maxScore: 28,
  },
  {
    name: '过度警觉',
    icon: 'mdi:activity',
    description: '易怒、惊跳反应、失眠',
    score: clusterE.value,
    maxScore: 24,
  },
])
...
const calculateResult = (answers: Record<number, number>): Result => {
  ...
  if (score <= 20) {
    ...
    const extra = meetsDSM5
      ? ' 你的症状模式符合DSM-5 PTSD诊断算法（B≥1, C≥1, D≥2, E≥2）。'
      : ''
    return {
      level: '重度症状',
      color: '#ef233c',
      ...
      analysis:
        '总得分 ≥51 分属于重度PTSD症状范围。在这个水平，症状可能导致严重的功能障碍、生活质量下降和共病风险增高。重度PTSD需要系统、专业的治疗干预。' + extra,
      suggestions: [
        '请尽快就诊精神科或创伤专科',
        '如实告诉医生你的症状严重程度和持续时间',
        '考虑住院治疗或强化门诊治疗选项',
