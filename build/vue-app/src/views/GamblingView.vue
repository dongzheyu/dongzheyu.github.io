<template>
  <TestTemplate
    title="赌博障碍自评测试"
    subtitle="赌博问题严重性指数（PGSI） · 9 道题 · 约 3 分钟"
    description="赌博问题严重性指数（Problem Gambling Severity Index）是评估赌博问题严重程度的标准化工具。请根据过去一年的情况回答。"
    instruction="在过去一年中，以下情况发生的频率如何？"
    :questions="questions"
    :options="freqOptions"
    :calculate-result-fn="calculateResult"
    theme-class="gambling-page"
    hero-padding="5%"
    instruction-padding="5%"
    disclaimer="PGSI 是筛查工具，不能替代专业诊断。赌博障碍是一种可治疗的精神障碍，通过认知行为疗法和支持团体通常能够显著改善。如果你或你认识的人正在与赌博问题作斗争，寻求帮助是康复的第一步。"
  />
</template>
...
const calculateResult = (answers: Record<number, number>): Result => {
  ...
  if (score === 0) {
    ...
    return {
      ...
      analysis:
        'PGSI 得分 1-2 分属于低风险范围。你可能偶尔赌博，但尚未出现明显的问题。建议设定明确的赌博限制，避免进一步发展。',
      suggestions: [
        '设定具体的赌博时间和金额限制',
        '避免使用信用卡或借款赌博',
        '寻找替代的娱乐活动',
    ...
    return {
      ...
      analysis:
        'PGSI 得分 3-7 分达到中等风险标准。在这个水平，赌博可能已经开始影响你的生活和心理健康。赌博障碍是可以通过治疗有效管理的。',
      suggestions: [
        '建议预约心理咨询师或成瘾专科医生',
        '参加赌博问题支持团体',
        '使用自我排除工具禁止进入赌场或赌博网站',
    ...
    return {
      ...
      analysis:
        'PGSI 得分 ≥8 分属于问题赌博（高风险）范围。在这个水平，赌博很可能已经严重影响你的生活、健康和人际关系。赌博障碍需要系统的专业治疗干预。',
      suggestions: [
        '请尽快就诊成瘾医学专科或精神科',
        '如实告诉医生你的赌博情况和财务问题',
        '考虑住院治疗或强化门诊治疗选项',
        '危机时刻请拨打赌博问题援助热线 400-161-9995',
      ],
    }
  }
}
